...
class log_stats():
    ...
    def parse_file( self ):
        ...
        try:
            _file = gzip.open( self.filename, 'r') if '.gz' in self.filename else open(self.filename, 'r' )
            _log = _file.read()
    ...
    def print_counts(self, cdict, title ):
        ...
        for i, t in enumerate( cdict.most_common( self.topn ) ):
            if 'IP Addresses' in title:
                print( '%s) %25s: %8d' %( i + 1, t[0][:40], t[1] ) )
            else:
                print( '%s) %25s: %8d' %( i + 1, t[0], t[1] ) )
...
def parse_arguments( ):
    ...
    if not any( 'auth.log' in _args for _args in sys.argv ):
...
if __name__ == '__main__':
    ...
    log_obj = log_stats( args.filename.name, args.n, args.full, args.clean, args.debug )
